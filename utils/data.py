...
class Redis(object):
    """Class that creates a Redis connection."""
    ...
    async def create(self):
        """Function that creates a redis connection."""
        self.instance = await aioredis.create_redis_pool(
            self.redis_url,
            encoding='utf-8'
        )

    async def get_storage(self, guild):
        """"Function that returns a specific redis instance for a guild."""
        return Storage(f'guild_{guild.id}:', self.instance)

class Storage():
    """Redis storage class for the bot."""
